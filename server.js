...
// Verify item ID in updated item object matches endpoint url
// Log errors for both
// Otherwise call `ShoppingList.update` with updated item
...
app.put('/recipes/:id', jsonParser, (req, res) => {
  ...
  if (req.params.id !== req.body.id) {
    const message = `Request path id (${request.params.id}) and request body id (${req.body.id}) must match`;
    console.error(message);
    return res.status(400).send(message);
  }
